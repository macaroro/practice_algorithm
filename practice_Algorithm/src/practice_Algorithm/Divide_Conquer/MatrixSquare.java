...
public class MatrixSquare {
	...
	public static int N;//행렬의 크기
	public static int[][] list;	// A^1 일 때의 배열(초기 배열)
	...
	public static void main(String[] args) throws IOException {
		...
		long B = Long.parseLong(st.nextToken());	// 타입 주의(숫자의 범위가 크기때문)
		...
		for(int i = 0; i < N; i++) {
			...
			for(int j = 0; j < N; j++) {
				 ...
				 * 초기화를 해준다.
				 */
				list[i][j] = Integer.parseInt(st.nextToken()) % MOD;
			}			
		}
	...
	public static int[][] pow(int[][] A, long exp) {
		...
		ret = multiply(ret, ret);//지수가 짝수라면
		
		// 만약 지수가 홀수라면 마지막에  A^1 (origin)을 곱해준다.
		if(exp % 2 == 1L) {
			ret = multiply(ret, list);
		}
